#!/usr/bin/env python3
"""stepper_odom.py — ROS 2 odometry publisher, driven by /stepperLeft + /stepperRight.

Subscribes to:
  /stepperLeft  (std_msgs/Int32) — signed step count delta from left  motor driver
  /stepperRight (std_msgs/Int32) — signed step count delta from right motor driver
...
Design
------
Callbacks ONLY accumulate steps into a buffer — they never integrate or drain.
The 50 Hz timer drains both buffers atomically, integrates, and publishes.
This avoids deadlocks and prevents one wheel's count being lost when the
other wheel's callback drains the buffer first.
"""
...
class StepperOdomNode(Node):
    ...
    def __init__(self):
        ...
        self._left_steps_buf  = 0    # accumulates between timer ticks
        ...
        # ---- subscribers — accumulate only ------------------------------------
        self.create_subscription(Int32, '/stepperLeft',  self._left_cb,  10)
        self.create_subscription(Int32, '/stepperRight', self._right_cb, 10)
    ...
    # ---- callbacks: ACCUMULATE ONLY, no draining, no integration -------------
    ...
    # ---- timer: drain → integrate → publish ----------------------------------

    def _timer_cb(self):
        now = self.get_clock().now()

        with self._lock:
            # Drain buffers atomically so no steps are lost
        ...
        odom.pose.covariance[35] = 1e6    # yaw   — IMU owns this

        # Twist covariance — EKF fuses vx only (odom0_config)
        odom.twist.covariance[0]  = 0.001  # vx  ← only value EKF uses
        odom.twist.covariance[7]  = 1e6    # vy  (non-holonomic — unused)
        odom.twist.covariance[35] = 1e6    # vyaw — IMU owns this
